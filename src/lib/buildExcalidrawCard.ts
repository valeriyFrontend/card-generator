...
function estimateLinesFallback(
  text: string,
  maxWidthPx: number,
  fontSize: number,
): number {
...
export async function buildExcalidrawCard(
  ...
  const titleText = isEvent
    ...
    : input.title.trim() || " ";
  const titleLines = wrapTextToLines(titleText, titleW, 32, 400);
  ...
  const textAvailableW =
    imageW > 0 ? CARD_W - (textX - ox) - PAD_X : CARD_W - 2 * PAD_X;
 ...
 */
export function assertExcalidrawClipboardValid(
  payload: ExcalidrawClipboard,
): void {
...
export async function copyCardToClipboard(
  ...
  const vendorBlob = () =>
    new Blob([json], { type: MIME_EXCALIDRAW_CLIPBOARD });
